...
const
  ...
  CHIP_SRAM_BASE = $40000;

  RAM_SIZE = $80000;
  RAM_CHOW_DIGITS = 5; // IntToHex(addr, 4)
  RAM_MASK_ADDR = $7ffff;

  BOOT_BASE_ADDR = $40000;
  BOOT_LOAD_ADDR = $0;
  BOOT_EXT_ADDR = 4; // +4 от старта (BOOT_LOAD_ADDR + 4)
  BOOT_EXT_LEN = 16;
  BOOT_BUF_LEN = 8192;
...
//    procedure CloseChkGPIO;
  ...
  private
    ...
    gpio : t_gpio;
    cntrun : dword;
    swdata : array[0..(RAM_SIZE-1)] of byte;
...
var
  frmMain: TfrmMain;
  FlashLoaderFname : string = 'floader825x.bin';
...
//        if GetDevVersion then begin
          ...
          flgReadGpioOn := False;
          PurgeCom(PURGE_TXCLEAR or PURGE_RXCLEAR);
          if not StmCmdByte(8, 4) then begin
...
begin
     ...
     buftx[3] := Addr;
     buftx[4] := $80;
     FillChar(buftx[5], DataLen + 1, $ff);
     if not WriteCom(@buftx, DataLen+6) then begin
//         StatusBar.Panels[1].Text:='Ошибка записи в '+ sComNane+'!';
         exit;
     end;
     Application.ProcessMessages;
     if not ReadCom(@bufrx, (DataLen+6)*2) then begin
//         StatusBar.Panels[1].Text:='Ошибка чтения в '+ sComNane+'!';
     ...
     or (bufrx[8] <> 0)
     or (bufrx[9] <> buftx[4])
     or (bufrx[(5+DataLen)*2] <> 2)
     or (bufrx[(5+DataLen)*2 + 1] <> $ff) then begin
//         StatusBar.Panels[1].Text:='Ошибки в считанном блоке! (нарушение синзронизации?)';
       exit;
     end;
     i := 0;
     while i < DataLen do begin
       if bufrx[(i + 5)*2] <> 0 then begin
//            StatusBar.Panels[1].Text:='Ошибки в считанных данных!';
         exit;
       end;
//       if (Addr + i < RAM_SIZE) then
         swdata[(Addr + i) and RAM_MASK_ADDR] := bufrx[(i + 5)*2 + 1];
       i := i + 1;
     end;
     result := True;
end;
...
//  flgValueChg := True;
  ...
  ReadIni;
  cntrun := 0;
  ButtonRdAll.Hint := 'Считать сегмент регистров чипа ('+ IntToStr(CHIP_REGS_SIZE div 1024) + ' кбайт) в файл';
  ButtonRdSram.Hint := 'Считать сегмент SRAM чипа ('+ IntToStr(CHIP_SRAM_SIZE div 1024) + ' кбайт) в файл';
...
begin
    MenuDisable;
    regaddr := Str2dword(EditRegAddr.Text) and RAM_MASK_ADDR;
    EditRegAddr.Text := '0x'+IntToHex(regaddr, RAM_CHOW_DIGITS);
//    regdata := Str2dword(EditRegData.Text);
//    EditRegData.Text := '0x'+IntToHex(regdata,8);
    StatusBar.Panels[1].Text:='Чтение адреса 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS);
    if not SwireRead(regaddr, 4) then begin
         StatusBar.Panels[1].Text:='Ошибка чтения по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS)+' !';
         ShowMessage(StatusBar.Panels[1].Text);
         MenuEnable;
         exit;
    end;
    ...
    EditRegData.Text := '0x'+IntToHex(regdata,8);
    StatusBar.Panels[1].Text:='Считано 0x'+ IntToHex(regdata, 8)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS);
    MenuEnable;
end;
...
begin
    ...
    regdata := Str2dword(EditRegData.Text);
    EditRegData.Text := '0x'+IntToHex(regdata,8);
    StatusBar.Panels[1].Text:='Запись 0x'+ IntToHex(regdata, 8)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS) + '...';
    if not SwireWrite(regaddr, regdata, 4) then begin
         StatusBar.Panels[1].Text:='Ошибка записи 0x'+ IntToHex(regdata, 8)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS)+' !';
         ShowMessage(StatusBar.Panels[1].Text);
         MenuEnable;
         exit;
    end;
    StatusBar.Panels[1].Text:='Записано 0x'+ IntToHex(regdata, 8)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS) + '.';
    MenuEnable;
end;
...
begin
  MenuDisable;
  ldaddr := Str2dword(EditLdAddr.Text) and RAM_MASK_ADDR;
  sldaddr := ldaddr;
  EditLdAddr.Text := '0x'+IntToHex(ldaddr, RAM_CHOW_DIGITS);
...
{$I-}
    ...
    while wrfsize < fsize do begin
      ...
      StatusBar.Panels[1].Text:='Запись '+ IntToStr(segsize)+' байт по адресу 0x'+ IntToHex(ldaddr, RAM_CHOW_DIGITS) + '...';
      if not SwireWrBuf(ldaddr, @swdata[ldaddr], segsize) then begin
         StatusBar.Panels[1].Text:='Ошибка записи по адресу 0x'+ IntToHex(ldaddr, RAM_CHOW_DIGITS)+' !';
         ShowMessage(StatusBar.Panels[1].Text);
         MenuEnable;
         exit;
      end;
    ...
    StatusBar.Panels[1].Text:='Записано '+ IntToStr(wrfsize)+' байт по адресу 0x'+ IntToHex(sldaddr, RAM_CHOW_DIGITS) + '. (' + IntToStr(ConnectTime) + ' мс)';
  end;
  MenuEnable;
end;
...
begin
  ...
  SigBreak := False;
     ConnectStartTime := GetTickCount();
     StatusBar.Panels[1].Text:='Чтение All Registers...';
     Addr := 0;
     while Addr < CHIP_REGS_SIZE do begin
       StatusBar.Panels[1].Text:='Чтение блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+'..';
       if not SwireRead(Addr, 32) then begin
         StatusBar.Panels[1].Text:='Ошибки чтения блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+' !';
         ShowMessage('Ошибки чтения блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+' !');
         MenuEnable;
         exit;
       end;
       if SigBreak then begin
         StatusBar.Panels[1].Text:='Чтения блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+' прервано!';
         SigBreak := False;
         MenuEnable;
         exit;
       end;
     ...
     ConnectTime := GetTickCount() - ConnectStartTime;
     StatusBar.Panels[1].Text:='Чтение '+IntToStr(CHIP_REGS_SIZE)+' байта - ок. (' + IntToStr(ConnectTime) + ' мс)';
     with SaveDialog do begin
      FilterIndex:=0;
      FileName := 'swregsdata.bin';
      InitialDir := '.';
      DefaultExt := 'bin';
     ...
     if SaveDialog.Execute then begin
      ...
      if bfile<>INVALID_HANDLE_VALUE then FileWrite(bfile,swdata[0],CHIP_REGS_SIZE);
      FileClose(bfile);
     end;
  MenuEnable;
end;
...
begin
    ...
    // 1.14 сек 10000 cnt при speed_stm = 3 -> tc = 114 us
    if cnt > 0 then begin
      asteps := (cnt * 1000) div 114;
      if asteps > 65535 then  cnt := 65535;
    end
    else asteps := 1;
    // Activate
    buftx[0] := $55;
    ...
    // 02 5A 00 00 00 00 00 B2 00 80 00 08 02 FF
    if not ReadCom(@bufrx, 14) then begin
    ...
    StatusBar.Panels[1].Text:='Запись 0x'+ IntToHex(speed_sws, 2)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS) + '...';
    if not SwireWrite(regaddr, speed_sws, 1) then begin
         StatusBar.Panels[1].Text:='Ошибка записи 0x'+ IntToHex(speed_sws, 2)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS)+' !';
         ShowMessage(StatusBar.Panels[1].Text);
         exit;
    end;
...
begin
    ...
    StatusBar.Panels[1].Text:='Записано 0x'+ IntToHex(speed_sws, 2)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS) + '.';
    MenuEnable;
end;
...
begin
    ...
    while buftx[2] < stop do begin
      StatusBar.Panels[1].Text:='Запись 0x0080'+ IntToHex(buftx[2], 2)+'00 по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS) + '...';
      if not SwireWrBuf(regaddr, @buftx[0], 4) then begin
         StatusBar.Panels[1].Text:='Ошибка записи 0x'+ IntToHex(buftx[2], 2)+' по адресу 0x'+ IntToHex(regaddr, RAM_CHOW_DIGITS)+' !';
         ShowMessage(StatusBar.Panels[1].Text);
         exit;
      end;
...
var
  ...
  fsize: Cardinal;
begin
    ldaddr :=  BOOT_BASE_ADDR + BOOT_LOAD_ADDR;
    sldaddr := ldaddr;
    result := False;
...
{$I-}
    ...
    StatusBar.Panels[1].Text:='Записано '+ IntToStr(wrfsize)+' байт по адресу 0x'+ IntToHex(sldaddr, RAM_CHOW_DIGITS) + '.';

    result := { SwireWrite($0602, $5, 1) and } SwireWrite($0602, $88, 1); // CPU start
    ...
    sldaddr := sldaddr and RAM_MASK_ADDR;
    ext_addr := swdata[FlashLoaderExtpAddr+0+sldaddr]
    or (swdata[FlashLoaderExtpAddr+1+sldaddr] shl 8)
    or (swdata[FlashLoaderExtpAddr+2+sldaddr] shl 16)
    or (swdata[FlashLoaderExtpAddr+3+sldaddr] shl 24) ;
    StatusBar.Panels[1].Text:='FlashLoader ExtAddr = 0x'+ IntToHex(ext_addr, 8);;

    ext_addr := ext_addr and RAM_MASK_ADDR;
    ...
    if (ext.iack <> ext.oack) or (ext.oack = 0) then begin
       StatusBar.Panels[1].Text:='FlashLoader не запускается!';
       ShowMessage(StatusBar.Panels[1].Text);
       result := False;
       exit;
    end;
    ...
    if ext.count < BOOT_BUF_LEN then begin
       StatusBar.Panels[1].Text:='Мал буфер у FlashLoader!';
       ShowMessage(StatusBar.Panels[1].Text);
       result := False;
    end;
...
//         StatusBar.Panels[1].Text:='Чтение блока по адресу 0x'+ IntToHex(fcaddr, 6)+'..';
         if not SwireRead((ext.pbuf and RAM_MASK_ADDR) + bufaddr, blksz) then begin
       ...
       CopyMemory(@fldata[fcaddr], @swdata[ext.pbuf and RAM_MASK_ADDR], buflen);
       faddr := faddr + buflen;
    end;
    ...
    while wrfsize < fsize do begin
      ...
      if flg then begin
        ...
        while sldaddr < segsize do begin
          if not SwireWrBuf((ext.pbuf and RAM_MASK_ADDR) + sldaddr, @fldata[faddr + sldaddr], blksz) then begin
...
procedure TfrmMain.ButtonGetPCClick(Sender: TObject);
var
  regaddr, regdata : DWORD;
begin
    MenuDisable;
    ...
    regdata := swdata[regaddr] or (swdata[regaddr+1] shl 8) or (swdata[regaddr+2] shl 16) or (swdata[regaddr+3] shl 24);
    StatusBar.Panels[1].Text:='CPU PC: 0x'+ IntToHex(regdata, 8);
    MenuEnable;
end;

procedure TfrmMain.ButtonRdSramClick(Sender: TObject);
var
Addr : Dword;
bfile : THandle;
begin
  ...
  SigBreak := False;
     ...
     StatusBar.Panels[1].Text:='Чтение All Registers...';
     Addr := CHIP_SRAM_BASE;
     while Addr < CHIP_SRAM_BASE + CHIP_SRAM_SIZE do begin
       StatusBar.Panels[1].Text:='Чтение блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+'..';
       if not SwireRead(Addr, 32) then begin
         StatusBar.Panels[1].Text:='Ошибки чтения блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+' !';
         ShowMessage('Ошибки чтения блока по адресу 0x'+ IntToHex(Addr, RAM_CHOW_DIGITS)+' !');
     ...
     ConnectTime := GetTickCount() - ConnectStartTime;
     StatusBar.Panels[1].Text:='Чтение '+IntToStr(CHIP_SRAM_SIZE div 1024)+' кбайта - ок. (' + IntToStr(ConnectTime) + ' мс)';
     with SaveDialog do begin
      FilterIndex:=0;
      FileName := 'swsram.bin';
      InitialDir := '.';
     ...
     if SaveDialog.Execute then begin
      Repaint;
      bfile:=FileCreate(SaveDialog.FileName);
      if bfile<>INVALID_HANDLE_VALUE then FileWrite(bfile,swdata[CHIP_SRAM_BASE],CHIP_SRAM_SIZE);
      FileClose(bfile);
     end;
  MenuEnable;
end;
